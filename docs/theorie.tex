...
Dazu kommt, dass Unity im Unterschied zu anderen (teils kostenpflichtigen) Programmen recht einfach zu handhaben ist. Durch diese Einfachheit büsst es aber nichts an Optionen und Möglichkeiten ein. Deshalb habe ich mich für Unity als Programmierumgebung meiner Maturaarbeit entschieden. Die Umgebung ist folgendermassen aufgebaut\footfullcite{unity3doverview}:
...
Für die Versionsverwaltung verwendete ich das weitverbreitete Programm Git\footfullcite{git}. Als Oberfläche kam Sourcetree\footfullcite{sourcetree} zum Einsatz.
Ich erstellte mir einen Studentenzugang auf Github\footfullcite{github}.
...
\lstinline{Start()} wird zu Laufzeitbeginn einmal aufgerufen.
...
Der \lstinline{Inputmanager}\footfullcite{unity3dinputmanager} dient der virtuellen Steuerung des Spielers. Er kann z.B. nach Achsen (vorwärts, seitwärts) abgefragt werden und deren Intensität mitliefern.
...
Jedes GameObject, auf welches sich physikalische Kräfte wie Gravitation auswirken, besitzt einen Rigidbody (=starrer Körper) \footfullcite{unity3drigidbodyoverview}.
\begin{figure}[H]
